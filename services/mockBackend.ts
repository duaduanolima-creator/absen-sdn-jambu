...
export const fetchGlobalRecords = async (): Promise<AttendanceRecord[]> => {
  ...
  try {
    const response = await fetch(`${REKAP_SHEET_URL}&t=${Date.now()}`); 
    ...
    if (firstRowCols.some(c => c.includes('nama') || c.includes('date') || c.includes('tanggal'))) {
        ...
        idx = {
            userId: 4, userName: 5, date: 1, time: 2, action: 3, locationLat: 7, locationLong: 8, notes: 6, location: -1, sppdActivity: -1, sppdDest: -1
        };
    }

    if (idx.date === -1) return getAllRecords();
    ...
    dataLines.forEach(line => {
        ...
        if (!isHeaderRow && cols[6] && cols[6].length < 10) name = `${name}, ${cols[6]}`; 

        if (!recordMap.has(key)) {
            recordMap.set(key, { id: key, userId, userName: name, date, type: 'present', location: '', notes: '' });
        }
        ...
        if (action.includes('checkin') || action.includes('datang') || action.includes('masuk')) {
            ...
            record.type = 'present';
            if (!isHeaderRow && cols[9]) notes = notes ? `${notes} (Jarak: ${cols[9]})` : `Jarak: ${cols[9]}`;
            ...
            record.sppdData = { activityType: activity, activityDetail: notes, destination: dest, startDate: date, endDate: date, resultReport: '', attachments: [] };
        }
    });
...
const fetchUsersFromSheet = async (): Promise<MockUser[] | null> => {
    ...
    try {
        ...
        const idx = { id: headers.indexOf('id'), name: headers.indexOf('name'), role: headers.indexOf('role'), nip: headers.indexOf('nip'), username: headers.indexOf('username'), password: headers.indexOf('password'), gender: headers.indexOf('gender'), status: headers.indexOf('employmentstatus'), position: headers.indexOf('position'), unit: headers.indexOf('workunit'), avatar: headers.indexOf('avatar') > -1 ? headers.indexOf('avatar') : headers.indexOf('foto') };
        if (idx.username === -1 || idx.password === -1) return null;
        const parsedUsers: MockUser[] = lines.slice(1).map(line => {
            ...
            return { id: cols[idx.id] || Math.random().toString(36).substr(2, 9), name: cols[idx.name] || 'Tanpa Nama', role: (isPrincipal ? 'principal' : 'teacher') as UserRole, nip: cols[idx.nip] || '', username: cols[idx.username] || '', password: cols[idx.password] || '', gender: (isFemale ? 'female' : 'male'), employmentStatus: cols[idx.status] || '', position: cols[idx.position] || '', workUnit: cols[idx.unit] || 'SDN JAMBU', avatar: idx.avatar > -1 ? cols[idx.avatar] : undefined };
        });
...
export const loginUser = async (username: string, password: string, requiredRole?: UserRole): Promise<User | null> => {
  ...
  if (sheetUsers && sheetUsers.length > 0) { localStorage.setItem(USERS_KEY, JSON.stringify(sheetUsers)); } else { initializeData(); }
  await new Promise(resolve => setTimeout(resolve, 800));
  const users: MockUser[] = JSON.parse(localStorage.getItem(USERS_KEY) || '[]');
  const user = users.find(u => u.username.toLowerCase() === username.toLowerCase() && u.password === password);
  if (user) {
    if (requiredRole) { if (requiredRole === 'teacher') { if (user.role !== 'teacher' && user.role !== 'principal') return null; } else { if (user.role !== requiredRole) return null; } }
...
export const getUserHistory = (userId: string): AttendanceRecord[] => {
  const records: AttendanceRecord[] = JSON.parse(localStorage.getItem(ATTENDANCE_KEY) || '[]');
  return records.filter(r => r.userId === userId).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
};
...
export const markCheckIn = async (user: User, location: string, photo: string, distance: string): Promise<AttendanceRecord> => {
  ...
  if (!deviceCheck.allowed) throw new Error(deviceCheck.message);
  const existingRecord = getTodayRecord(user.id);
  if (existingRecord) {
     if (existingRecord.type === 'present' && existingRecord.checkInTime) throw new Error("Anda sudah melakukan absen datang hari ini.");
     if (['sick', 'leave', 'sppd'].includes(existingRecord.type)) throw new Error("Anda sudah tercatat Izin/Sakit/SPPD hari ini.");
  }
  await new Promise(resolve => setTimeout(resolve, 800));
  ...
  const newRecord: AttendanceRecord = { id: `${user.id}-${today}`, userId: user.id, userName: user.name, date: today, checkInTime: time, type: 'present', location, photo, notes };
  saveAttendance(newRecord);
  registerDeviceUsage(user.id);
  postToGAS({ action: 'checkin', typeLabel: 'Datang', userId: user.id, userName: user.name, nip: user.nip || '-', date: today, time: time, location, distance, photo, notes });
  return newRecord;
};
...
export const markCheckOut = async (user: User, location: string, photo: string, distance: string): Promise<AttendanceRecord | null> => {
  ...
  const today = new Date().toISOString().split('T')[0];
  const updatedRecord = { ...record, checkOutTime: time, location };
  saveAttendance(updatedRecord);
  postToGAS({ action: 'checkout', typeLabel: 'Pulang', userId: user.id, userName: user.name, nip: user.nip || '-', date: today, time, location, distance, photo });
  return updatedRecord;
};

export const submitReport = async (user: User, type: AttendanceType, notes: string, sppdData?: SppdData, attachment?: string, startDate?: string, endDate?: string): Promise<AttendanceRecord> => {
  ...
  if (existing) {
    recordToSave = { ...existing, type: sppdData ? 'sppd' : (type === 'present' ? existing.type : type), notes: notes || existing.notes, sppdData: sppdData || existing.sppdData, attachment: attachment || existing.attachment, leaveStartDate: startDate || existing.leaveStartDate, leaveEndDate: endDate || existing.leaveEndDate };
  } else {
    recordToSave = { id: `${user.id}-${today}`, userId: user.id, userName: user.name, date: today, checkInTime: time, type: sppdData ? 'sppd' : type, notes, sppdData, attachment, leaveStartDate: startDate, leaveEndDate: endDate };
  }
  saveAttendance(recordToSave);
  if (type === 'sppd' && sppdData) { postToGAS({ action: 'sppd', userId: user.id, userName: user.name, nip: user.nip || '-', sppd: sppdData, attachments: sppdData.attachments }); } else if (type === 'sick' || type === 'leave') { postToGAS({ action: type, userId: user.id, userName: user.name, nip: user.nip || '-', startDate, endDate, notes, attachment }); }
  return recordToSave;
};

export const downloadMonthlyReport = (records: AttendanceRecord[], startDate: string, endDate: string) => {
  // Sekarang mengambil records dari parameter (data hasil spreadsheet), bukan lagi LocalStorage secara paksa
  ...
  const header = ['Tanggal', 'Nama Guru', 'NIP/ID', 'Status', 'Masuk', 'Pulang', 'Lokasi', 'Catatan/Info SPPD', 'Mulai Izin', 'Selesai Izin'].map(escapeCsv);
  const rows = filteredRecords.map(r => {
    let noteContent = r.notes || '';
    if (r.sppdData) noteContent = `SPPD: ${r.sppdData.activityType} di ${r.sppdData.destination}`;
    return [escapeCsv(r.date), escapeCsv(r.userName), escapeCsv(r.userId), escapeCsv(r.type.toUpperCase()), escapeCsv(r.checkInTime || '-'), escapeCsv(r.checkOutTime || '-'), escapeCsv(r.location || '-'), escapeCsv(noteContent), escapeCsv(r.leaveStartDate || (r.sppdData ? r.sppdData.startDate : '-')), escapeCsv(r.leaveEndDate || (r.sppdData ? r.sppdData.endDate : '-'))];
  });
